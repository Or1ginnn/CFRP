...
Persistent plan 是 controller-owned 的可执行状态，而不是 CoT。Stage 1 中它只读：
`progress=advance` 仅允许推进 `current -> done`、`next todo -> current`；不允许
`plan_update`、`tool`、`replan` 或 `risk` language token。
...
- shortest-path oracle 轨迹采集，及从其生成的 75 条多图 SFT manifest；
- 可分片 warm-up collector、SFT manifest validator、Qwen3-VL LoRA SFT training
  preflight。

这 75 条是端到端格式 smoke，不是正式训练集，也不是 Phase 1 的 on-policy risk
数据。它们的 action 来自 Habitat shortest-path oracle；plan/progress 是 instruction
拆分和确定性标注，仅用于 normal-policy warm-up。
...
它必须能够读取 compact plan 和固定视觉/动作历史，但没有 risk head、tool、
recovery 或 structural plan update。

### 剩余执行项

1. 以 `train` split 的确定性 shards 采集 normal-policy warm-up 数据；保留数据
   manifest、collector commit、split、seed、episode range 和配置。
2. 每个 shard 转为多图 Qwen3-VL SFT JSONL，并在训练前检查 XML、图像 URI、
   action 分布和缺失文件。
