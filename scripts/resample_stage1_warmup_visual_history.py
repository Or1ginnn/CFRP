"""Rebuild Stage 1 frame references as eight route anchors plus current RGB.

This data-only migration reuses retained ``frame-XXXX.npy`` files. It does not
run Habitat and does not change expert actions, plans, progress, or subgoals.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--input-dir", required=True, help="Complete merged warm-up directory")
...
def resample_warmup_directory(input_dir: Path, output_dir: Path) -> dict[str, Any]:
    ...
    count = 0
    with source_records.open("r", encoding="utf-8") as source, destination.open(
        "w", encoding="utf-8"
    ) as target:
...
def resample_warmup_record(
    record: Mapping[str, Any], *, line_number: int = 0
) -> dict[str, Any]:
    """Replace only visual_history_paths using retained episode RGB frames."""
    ...
    current = frames_dir / f"frame-{turn_index:04d}.npy"
    historical = tuple(
        frames_dir / f"frame-{index:04d}.npy" for index in range(turn_index)
    )
    required = historical + (current,)
    missing = next((str(path) for path in required if not path.is_file()), None)
    if missing is not None:
        raise FileNotFoundError(_prefix(line_number) + "missing retained raw frame: " + missing)

    visible = select_temporal_history(historical + (current,))
    if len(visible) > DEFAULT_HISTORY_ANCHOR_COUNT + 1 or visible[-1] != current:
        raise RuntimeError(_prefix(line_number) + "invalid streaming visual resample")
    request["visual_history_paths"] = [str(path) for path in visible]
